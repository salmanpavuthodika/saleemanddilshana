...
export function ProphetDuaCard() {
  return (
    <motion.div
      ...
      className="relative max-w-2xl mx-auto px-6 py-8 my-12 bg-gradient-to-br from-[#FCFAF6] to-[#FAF5EC] border border-[#E3DAC9] rounded-2xl shadow-[0_15px_35px_rgba(163,117,109,0.04)] text-center overflow-hidden"
    >
      ...
      <div className="relative z-10 space-y-4">
        ...
        <h3 className="font-serif text-2xl md:text-3xl text-[#7E5E4E] leading-loose tracking-wide font-medium select-text">
          بَارَكَ اللَّهُ لَكُمَا وَبَارَكَ عَلَيْكُمَا وَجَمَعَ بَيْنَكُمَا فِي خَيْرٍ
        </h3>

        {/* Translation */}
        <p className="font-serif italic text-sm md:text-base text-[#4A3B32] max-w-lg mx-auto leading-relaxed">
          "May Allah bless you both, and shower His blessings upon you, and join you together in goodness."
        </p>

        <span className="block font-sans text-[9px] uppercase tracking-widest text-[#A68F80]">
  ...
  return (
    <div className="absolute inset-0 pointer-events-none select-none z-0 overflow-hidden">
      ...
      >
        <defs>
          <linearGradient id="left-curtain-grad" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <stop offset="15%" stopColor="#FAF5EC" />
            <stop offset="35%" stopColor="#DFD6C6" />
            <stop offset="55%" stopColor="#FAF7F2" />
            <stop offset="75%" stopColor="#DFD6C6" />
            <stop offset="90%" stopColor="#FCFAF6" />
            <stop offset="100%" stopColor="#C2A289" stopOpacity="0.3" />
          </linearGradient>
        </defs>
        ...
        <path d="M 100,420 L 105,432" stroke="#AA7C11" strokeWidth="2" />
        <path d="M 103,421 L 108,431" stroke="#AA7C11" strokeWidth="2" />
      </svg>
      ...
      >
        ...
        {/* Golden fringe detailing on tieback */}
        <path d="M 200,420 L 195,432" stroke="#AA7C11" strokeWidth="2" />
        <path d="M 197,421 L 192,431" stroke="#AA7C11" strokeWidth="2" />
      </svg>

      {/* 4c. Top Stage Canopy Valance Swag with Arches */}
      <div className="absolute top-0 inset-x-0 h-16 sm:h-24 bg-gradient-to-b from-[#EFE8DC] to-[#FAF5EC] border-b border-[#C2A289]/30 shadow-md">
        <div className="absolute inset-x-0 bottom-0 flex justify-between h-4">
          {/* Repeating arch peaks or drapes border */}
          {Array.from({ length: 15 }).map((_, i) => (
            <div 
              key={i} 
              className="flex-1 h-4 bg-[#FAF5EC] border-b border-r border-[#C2A289]/20 rounded-b-full shadow-inner"
        ...
        <div className="absolute -bottom-2.5 inset-x-0 flex justify-around">
          {Array.from({ length: 30 }).map((_, i) => (
            <div key={i} className="w-1 h-1 rounded-full bg-[#C2A289] opacity-75" />
          ))}
        </div>
      </div>
